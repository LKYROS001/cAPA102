...
currentframe = 40 #number of image frames in the frames file
with open('vid2.txt', 'w') as f: 
...
for v in range(1,currentframe): #loop through all image frames
  if (v<10):
    FILENAME = "frames2/ezgif-frame-00" + str(v)+".jpg"  # Image file to load
  else:
    FILENAME = "frames2/ezgif-frame-0" + str(v)+".jpg"  # Image file to load
...
#create array for final rgb value outputs
  ...
  with open('vid2.txt', 'a') as f: #print values to video file
    for n in range(361):
      for m in range(72):
